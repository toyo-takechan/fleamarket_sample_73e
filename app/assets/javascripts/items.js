...
$(document).on('turbolinks:load', ()=> {
  ...
  const buildFileField = (num)=> {
    const html = `<div data-index="${num}" class="js-file_group">
                    ...
                    id="item_images_attributes_${num}_image_url">
                    <br>
  ...
  // プレビュー用のimgタグを生成する関数
  const buildImg = (index, image_url)=> {
    const html = `<img data-index="${index}" src="${image_url}" width="100px" height="100px">`;
    return html;
  }
  ...
  $('.js-file_group').on('click', '.js-remove', function() {
    ...
    if ($('.js-file').length == 0) $('.js-file_group').append(buildFileField(fileIndex[0]));
  });
});